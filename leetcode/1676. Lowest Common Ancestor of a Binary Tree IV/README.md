...
public:
    ...
    int helper(TreeNode* root){
        ...
        int totalCount = l + r + sett.count(root);
        // the lowest level where all the required nodes are found
        if(totalCount==sett.size() and !LCA)
            LCA = root;
        return totalCount;
    }
    TreeNode* lowestCommonAncestor(TreeNode* root, vector<TreeNode*> &nodes) {
        for(auto n: nodes) sett.insert(n);
        helper(root);
        return LCA;
    }
};
