...
public:
    ...
    void solve(int start, int sum, int target, vector<int>temp, int k){
        if(sum==target and temp.size()==k){
            ans.push_back(temp);
            return;
        }
        for(int i=start; i<candidates.size(); i++){
            if(candidates[i]+sum<=target){
                ...
                solve(i+1, sum+candidates[i], target, temp, k);
                temp.pop_back();
            }
        }
        return; 
