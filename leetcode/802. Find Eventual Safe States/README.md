...
class Solution {
public:
    bool isNotCyclic(vector<vector<int>>&adj, int u, vector<int>&vis){
        if(vis[u]) return vis[u]==1; // return if already been visited
        vis[u] = 2; // 2 => unsafe
        for(auto v: adj[u]){
            if(!isNotCyclic(adj, v, vis))
                   return false; 
        }
        vis[u] = 1; // 1 => safe
        return true;
